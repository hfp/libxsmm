...
SRCFILES_LIB := $(patsubst %,$(ROOTSRC)/%, \
          ...
          libxsmm_rng.c libxsmm_mhd.c generator_gemm_reference_impl.c generator_mateltwise_reference_impl.c generator_matequation_reference_impl.c generator_x86_reference.c generator_aarch64_reference.c)
...
$(OUTDIR)/libxsmmext.$(DLIBEXT): $(OUTDIR)/libxsmm.$(DLIBEXT) $(OBJFILES_EXD)
	$(LIB_SOLD) $(call solink_version,$(OUTDIR)/libxsmmext.$(DLIBEXT)) $(OBJFILES_EXD) \
		$(call abslib,$(OUTDIR)/libxsmm.$(ILIBEXT)) $(call cleanld,$(LDFLAGS) $(CLDFLAGS) $(EXTLDFLAGS))
else
